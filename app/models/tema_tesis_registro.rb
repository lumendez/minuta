class TemaTesisRegistro < ApplicationRecord
  ...
  when /^created_at_/
    ...
    order("tema_tesis_registros.created_at #{ direction }")
  when /^apellido_/
    # Simple sort on the name colums
    order("LOWER(tema_tesis_registros.user.paterno) #{ direction }, LOWER(tema_tesis_registros.user.materno) #{ direction }")
  else
