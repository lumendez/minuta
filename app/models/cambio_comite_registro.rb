class CambioComiteRegistro < ApplicationRecord
  ...
  when /^created_at_/
    ...
    order("cambio_comite_registros.created_at #{ direction }")
  when /^apellido_/
    # Simple sort on the name colums
    order("LOWER(cambio_comite_registros.user.paterno) #{ direction }, LOWER(cambio_comite_registros.user.materno) #{ direction }")
  else
