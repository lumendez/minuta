class DirectorTesisRegistro < ApplicationRecord
  ...
  when /^created_at_/
    ...
    order("director_tesis_registros.created_at #{ direction }")
  when /^apellido_/
    # Simple sort on the name colums
    order("LOWER(director_tesis_registros.user.paterno) #{ direction }, LOWER(director_tesis_registros.user.materno) #{ direction }")
  else
