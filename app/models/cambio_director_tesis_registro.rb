class CambioDirectorTesisRegistro < ApplicationRecord
  ...
  when /^created_at_/
    ...
    order("cambio_director_tesis_registros.created_at #{ direction }")
  when /^apellido_/
    # Simple sort on the name colums
    order("LOWER(cambio_director_tesis_registros.user.paterno) #{ direction }, LOWER(cambio_director_tesis_registros.user.materno) #{ direction }")
  else
